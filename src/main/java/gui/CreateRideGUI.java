...
public class CreateRideGUI extends JFrame {
	...
	private JLabel jLabelSeats = new JLabel(ResourceBundle.getBundle(ApplicationLauncher.ET).getString("CreateRideGUI.NumberOfSeats"));
	private JLabel jLabRideDate = new JLabel(ResourceBundle.getBundle(ApplicationLauncher.ET).getString("CreateRideGUI.RideDate"));
	private JLabel jLabelPrice = new JLabel(ResourceBundle.getBundle(ApplicationLauncher.ET).getString("CreateRideGUI.Price"));
	...
	private JButton jButtonCreate = new JButton(ResourceBundle.getBundle(ApplicationLauncher.ET).getString("CreateRideGUI.CreateRide"));
	...
	private final JLabel lblNewLabelCar = new JLabel(ResourceBundle.getBundle(ApplicationLauncher.ET).getString("CreateRideGUI.Car")); //$NON-NLS-1$ //$NON-NLS-2$
	...
	public CreateRideGUI(Driver driver) {
		...
		this.setTitle(ResourceBundle.getBundle(ApplicationLauncher.ET).getString("CreateRideGUI.CreateRide"));
		...
		lblNewLabelCar.setBounds(6, 203, 123, 14);
		
		getContentPane().add(lblNewLabelCar);
		int i = 0;
		List<Car> s = facade.getCars(driver.getEmail());
		while(i<s.size()) {
			bookedRides.addElement(s.get(i));
			i++;
		}
		comboBoxCar = new JComboBox<Car>();
		comboBoxCar.setBounds(149, 199, 116, 22);
		getContentPane().add(comboBoxCar);
		comboBoxCar.setModel(bookedRides);
		 //Code for JCalendar
	...
	private void jButtonCreate_actionPerformed(ActionEvent e) {
		...
		if (error!=null) 
			...
			try {
				...
				jLabelMsg.setText(ResourceBundle.getBundle(ApplicationLauncher.ET).getString("CreateRideGUI.RideCreated"));

			} catch (RideAlreadyExistException | RideMustBeLaterThanTodayException e1) {
	...
	private String field_Errors() {
		...
		try {
			if ((fieldOrigin.getText().length()==0) || (fieldDestination.getText().length()==0) || (jTextFieldSeats.getText().length()==0) || (jTextFieldPrice.getText().length()==0))
				return ResourceBundle.getBundle(ApplicationLauncher.ET).getString("CreateRideGUI.ErrorQuery");
			else {
				...
				if (inputSeats <= 0) {
					return ResourceBundle.getBundle(ApplicationLauncher.ET).getString("CreateRideGUI.SeatsMustBeGreaterThan0");
				}
				else {
					float price = Float.parseFloat(jTextFieldPrice.getText());
					if (price <= 0) 
						return ResourceBundle.getBundle(ApplicationLauncher.ET).getString("CreateRideGUI.PriceMustBeGreaterThan0");
			...
			return  ResourceBundle.getBundle(ApplicationLauncher.ET).getString("CreateRideGUI.ErrorNumber");		
		} catch (Exception e1) {
