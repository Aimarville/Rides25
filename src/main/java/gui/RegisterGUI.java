...
public class RegisterGUI extends JFrame {
	...
	public RegisterGUI() {
		...
		setTitle(ResourceBundle.getBundle(ApplicationLauncher.ET).getString("RegisterGUI.Register"));
		...
		JLabel lblErabiltzaileIzenaSartu = new JLabel(ResourceBundle.getBundle(ApplicationLauncher.ET).getString("RegisterGUI.Email"));
		lblErabiltzaileIzenaSartu.setBounds(50, 29, 165, 16);
		contentPane.add(lblErabiltzaileIzenaSartu);
		
		JLabel lblPasahitzaSartu = new JLabel(ResourceBundle.getBundle(ApplicationLauncher.ET).getString("RegisterGUI.Pass"));
		lblPasahitzaSartu.setBounds(50, 72, 145, 16);
		contentPane.add(lblPasahitzaSartu);
		
		JLabel lblErabiltzaileMotaAukeratu = new JLabel(ResourceBundle.getBundle(ApplicationLauncher.ET).getString("RegisterGUI.DRorTR"));
		lblErabiltzaileMotaAukeratu.setBounds(30, 118, 165, 16);
		contentPane.add(lblErabiltzaileMotaAukeratu);
		
		btnSistemanSartu = new JButton(ResourceBundle.getBundle(ApplicationLauncher.ET).getString("RegisterGUI.Register"));
		btnSistemanSartu.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if(res) {
					textArea.setText(ResourceBundle.getBundle(ApplicationLauncher.ET).getString("RegisterGUI.Registered"));
				}else {
					textArea.setText(ResourceBundle.getBundle(ApplicationLauncher.ET).getString("RegisterGUI.Error"));
				}
			}
		});
		...
		rdbtnDriver = new JRadioButton(ResourceBundle.getBundle(ApplicationLauncher.ET).getString("RegisterGUI.Driver"));
		buttonGroup.add(rdbtnDriver);
		rdbtnDriver.setBounds(207, 114, 88, 23);
		contentPane.add(rdbtnDriver);
		
		rdbtnTraveler = new JRadioButton(ResourceBundle.getBundle(ApplicationLauncher.ET).getString("RegisterGUI.Traveler"));
