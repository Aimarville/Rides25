...
public class FindAndBookGUI extends JFrame {
	...
	private final JButton jButtonBook = new JButton(ResourceBundle.getBundle(ApplicationLauncher.ET).getString("FindAndBookGUI.Book")); //$NON-NLS-1$ //$NON-NLS-2$
	private JTextField textFieldSeats;


	public FindAndBookGUI(Passenger passenger)
	{
		...
		jButtonClose.setBounds(new Rectangle(172, 420, 130, 27));
		...
		jButtonBook.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String email = null;
				try {
					int row = tableRides.getSelectedRow();
					email = (String) tableModelRides.getValueAt(row, 0);
					int seats = (int) tableModelRides.getValueAt(row, 1);
					Float price = (Float) tableModelRides.getValueAt(row, 2);
				}catch (ArrayIndexOutOfBoundsException exc) {
					jLabelEvents.setText(ResourceBundle.getBundle(ApplicationLauncher.ET).getString("FindAndBookGUI.UnselectedError"));
				}
				BLFacade facade = HomeGUI.getBusinessLogic();
				
				int bookedseats = Integer.parseInt(textFieldSeats.getText());
				String origin = (String) jComboBoxOrigin.getSelectedItem();
				String dest = (String) jComboBoxDestination.getSelectedItem();
				...
				if(facade.bookRide(passenger, email, bookedseats, ride.getRideNumber())) {
					...
					jLabelEvents.setText(ResourceBundle.getBundle(ApplicationLauncher.ET).getString("FindAndBookGUI.ERROR"));
				}
				
			}
		});
		jButtonBook.setBounds(388, 420, 130, 27);
		
		getContentPane().add(jButtonBook);
		
		JLabel lblNewLabelSeatsBook = new JLabel(ResourceBundle.getBundle(ApplicationLauncher.ET).getString("FindAndBookGUI.BookedSeats")); //$NON-NLS-1$ //$NON-NLS-2$
		lblNewLabelSeatsBook.setBounds(10, 145, 169, 16);
		getContentPane().add(lblNewLabelSeatsBook);
		
		textFieldSeats = new JTextField();
		textFieldSeats.setBounds(189, 143, 86, 20);
